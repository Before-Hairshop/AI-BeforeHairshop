...
from secret import AWS_RESPONSE_SQS_URL, AWS_REQUEST_SQS_URL, AWS_S3_BUCKET_REGION, AWS_S3_BUCKET_NAME, USER_INPUT_IMAGE_PATH, PREPROCESSING_USER_INPUT_IMAGE_PATH
import logging
...
def main():
    ...
    while True:
        ...
        for message in messages['Messages']:
            ...
            image_path = USER_INPUT_IMAGE_PATH # 알맞게 이미지 path 수정할 것
            ...
            try:
                ...
                logger.exception("Send message failed! (Message body : { result : %s, user_id : %s , image_id : %s})", "이미지 회전 실패", param_user_id, image_id)
                pass
            ...
            # align 안되어있으면, 실패했다고 리턴함. (폴더에 한 개가 아닌 0개 혹은 2개 생성되면 실패)
            if len(os.listdir("/content/drive/MyDrive/Barbershop-Plus-Plus/Barbershop/input/face")) != 1:
                fail_body_json = {
                    'result' : 'fail',
                ...
                try:
                    ...
                    logger.info("Send message success! (Message body : { result : %s, user_id : %s , image_id : %s})", "success", param_user_id, image_id)
                    continue
                except ClientError as error:
                    logger.exception("Send message failed! (Message body : { result : %s, user_id : %s , image_id : %s})", "fail", param_user_id, image_id)
